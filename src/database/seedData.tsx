...
const EXPENSE_CATEGORIES = [
  ...
  { name: 'Косметика', icon: 'face-woman', color: '#FF69B4', type: 'expense', order: 16 },
  { name: 'Домашние животные', icon: 'dog', color: '#CD853F', type: 'expense', order: 17 },
  { name: 'Прочее', icon: 'dots-horizontal', color: '#95A5A6', type: 'expense', order: 18 },
];

const INCOME_CATEGORIES = [
  { name: 'Зарплата', icon: 'cash', color: '#2ECC71', type: 'income', order: 1 },
  { name: 'Фриланс', icon: 'laptop', color: '#3498DB', type: 'income', order: 2 },
  { name: 'Инвестиции', icon: 'trending-up', color: '#9B59B6', type: 'income', order: 3 },
  ...
  { name: 'Прочее', icon: 'dots-horizontal', color: '#95A5A6', type: 'income', order: 7 },
];

// Реалистичные цены для каждой категории расходов
const getExpenseAmountByCategory = (categoryName: string, note?: string): number => {
...
const getRandomDate = (): number => {
  ...
  return new Date(threeMonthsAgo.getTime() + Math.random() * (now.getTime() - threeMonthsAgo.getTime())).getTime();
};

// Описания расходов по категориям
const expenseNotesByCategory: Record<string, string[]> = {
  'Еда и рестораны': ['Обед в ресторане', 'Ужин в ресторане', 'Бизнес-ланч', 'Суши', 'Пицца'],
  'Продукты': ['Закупка продуктов', 'Овощи и фрукты', 'Мясные продукты', 'Молочные продукты'],
  ...
  'Связь и интернет': ['Мобильная связь', 'Домашний интернет', 'Подписка Netflix'],
...
const generateTestTransactions = (categories: any[]) => {
  ...
  for (const category of incomeCategories) {
    const transactionCount = category.name === 'Зарплата' ? 3 : 
                              category.name === 'Фриланс' ? 4 :
                              category.name === 'Инвестиции' ? 3 : 2;
  ...
  const recurringPayments = [
    ...
    { name: 'Интернет', amount: 850, categoryName: 'Связь и интернет', day: 10 },
    { name: 'Мобильная связь', amount: 450, categoryName: 'Связь и интернет', day: 15 },
    { name: 'Спортзал', amount: 2500, categoryName: 'Спорт и хобби', day: 8 },
    { name: 'Netflix', amount: 499, categoryName: 'Развлечения', day: 5 },
    { name: 'Электричество', amount: 1200, categoryName: 'Коммунальные услуги', day: 20 },
...
export async function seedTestData() {
  ...
  try {
    ...
    const transactions = generateTestTransactions(createdCategories);
    ...
    const budgets = generateTestBudgets(createdCategories);
